...
//-------------------------------------------------------------------------------------------------------------------------------------------
    ...
    // Συνάρτηση για την υποβολή του score
    document.getElementById('score-button').addEventListener('click', function(event) {
        event.preventDefault();

        const scoreInput = document.getElementById('score');
        const score = scoreInput.value;
        const email = document.getElementById('login-email').value; // Υποθέτουμε ότι το email είναι γνωστό μετά την είσοδο του χρήστη

        if (score.includes('.') || score.includes(',')) {  // Ελέγχει αν το score περιέχει τελεία ή κόμμα
            alert("Παρακαλώ εισάγετε έναν ακέραιο αριθμό. Δεν επιτρέπονται οι δεκαδικοί αριθμοί.");
            return;
        }
        
        if (isNaN(score) || score < 0 || score > 100 || score === '-0') {  // Ελέγχει αν το score είναι αριθμός και βρίσκεται μεταξύ 0 και 100
            alert("Παρακαλώ εισάγετε έναν αριθμό από 0 έως 100.");
            return;
        }
    
        if (!score) {
            alert("Παρακαλώ εισάγετε ένα score.");
            return;
        }
            ...
            if (snapshot.exists()) {
                snapshot.forEach(childSnapshot => {
                    ...
                    firebase.database().ref(`users/${userId}/scores`).push({ score }).then(() => {
                        ...
                        scoreInput.value = ''; // Διαγραφή της τιμής του input μετά την υποβολή
                        
                        // Προσθήκη του νέου score στην κορυφή του πίνακα
                        const scoreBody = document.getElementById('score-body');
                        const row = document.createElement('tr');
                        const cell = document.createElement('td');
                        cell.textContent = score;
                        row.appendChild(cell);
                        scoreBody.insertBefore(row, scoreBody.firstChild); // Εισαγωγή του νέου score στην κορυφή
                    }).catch(error => {
    ...
    function displayScores(userId) {
        const scoreBody = document.getElementById('score-body');
        scoreBody.innerHTML = ''; // Καθαρισμός του πίνακα πριν από την εμφάνιση των scores
    
        // Ανάκτηση των scores του χρήστη από τη βάση δεδομένων
        firebase.database().ref(`users/${userId}/scores`).once('value', snapshot => {
            snapshot.forEach(childSnapshot => {
                const scoreData = childSnapshot.val();
                const row = document.createElement('tr');
                const cell = document.createElement('td');
                cell.textContent = `${scoreData.score}`;
                row.appendChild(cell);
...
//  Όταν ο χρήστης επιλέγει το κουμπί 'Είσοδος'-----------------------------------------------------------------------------------------------------
    ...
    document.getElementById('submit-login-button').addEventListener('click', function(event) {
        ...
        console.log("Email που συνδέθηκε ο χρήστης: " + email);
        ...
        // Έλεγχος αν το email υπάρχει στη βάση δεδομένων
        checkIfUserExists(email, exists => {
            if (exists) {
                ...
                firebase.database().ref('users').orderByChild('email').equalTo(email).once('value', snapshot => {
                    if (snapshot.exists()) {
                        snapshot.forEach(childSnapshot => {
                            ...
                            document.getElementById('InsertScore').style.display = 'block';
